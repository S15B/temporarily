...
def euler(end):
    ...
    while t[-1] < end - h:
        t.append(t[-1] + h)
        tau = k[0] * y[-1][1] + k[1] * np.sin(y[-1][0]) + m*l*l*u
        y.append([0, y[-1][1] + h * f(y[-1], t[-1], tau)[1]])
        y[-1][0] = (y[-2][0] + h * f([y[-2][0], y[-1][1]], t[-1], tau)[0]) 
...
plt.xlabel('Время')
plt.ylabel('')
plt.plot(t, theta, t, omega, t, acceler)
plt.legend(['Положение', 'Скорость', 'Ускорение'])
